...
export const SidebarItem = forwardRef(function SidebarItem(
  ...
  let classes = clsx(
    // Base
    'flex w-full items-center gap-3 px-4 py-4 text-left text-4xl font-bold text-zinc-950 uppercase tracking-wider',
    // Leading icon/icon-only
    '*:data-[slot=icon]:size-6 *:data-[slot=icon]:shrink-0 *:data-[slot=icon]:fill-zinc-500',
    ...
    <span className={clsx(className, 'relative')}>
      {current && (
        <motion.span
          ...
          className="absolute inset-y-2 -left-4 w-1 bg-zinc-950 dark:bg-white"
        />
      )}
